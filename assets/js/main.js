    ...
    list.innerHTML = notifications.slice(0, 10).map(n => `
        <div class="notification-item ${n.type} ${n.read ? 'read' : 'unread'}" data-id="${n.id}" onclick="markNotificationAsRead('${n.id}')">
            <div class="notification-item-header">
                <i data-lucide="${iconMap[n.type] || 'info'}" class="notification-item-icon"></i>
...
function loadTransactionHistory() {
    ...
    // Use both sales transactions and wallet transactions
    const salesTransactions = AppData.transactions.map(t => ({
        ...t,
        description: t.items.map(item => `${item.name} (${item.quantity})`).join(', '),
        isSale: true
    }));
    
    const walletTransactions = AppData.wallet.transactions.map(t => ({
        ...t,
        description: t.description || 'Wallet Transaction',
        isSale: false
    }));
    
    allTransactions = [...salesTransactions, ...walletTransactions].filter(t => {
        const matchesSearch = t.description.toLowerCase().includes(searchTerm);
        const matchesFilter = filterType === 'all' || (t.isSale && filterType === 'sale') || (!t.isSale && filterType === 'expense');
...
function displayTransactionHistory() {
    ...
    list.innerHTML = paginatedTransactions.map(t => {
        ...
        const icon = t.isSale ? 'shopping-cart' : (isIncome ? 'arrow-down-circle' : 'arrow-up-circle');
        const iconClass = t.isSale ? 'income' : (isIncome ? 'income' : 'expense');
        ...
        <div class="transaction-item">
            ...
            <div class="transaction-amount ${amountColor}">${amountSign}${formatCurrency(Math.abs(t.amount))}</div>
            ${t.isSale ? `
